...
(use-package nov
  ...
  :hook (nov-mode . ss/nov-font-setup)
...
(use-package mu4e
  ...
  :config
  (dolist (bookmark '(
               (:query "maildir:/[Gmail]/All\\ Mail" :name "All Mail" :key ?a :favorite t)
               (:query "maildir:/[Gmail]/Drafts" :name "Drafts" :key ?d :favorite t)
               (:query "maildir:/[Gmail]/Sent\\ Mail" :name "Sent" :key ?s :favorite t)
               (:query "maildir:/[Gmail]/Trash" :name "Trash" :key ?t :favorite t)
